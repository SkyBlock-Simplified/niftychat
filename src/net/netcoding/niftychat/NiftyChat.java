...
public class NiftyChat extends BukkitPlugin {
	...
	private boolean setupTables() {
		try {
			getSQL().createTableAsync(Config.FORMAT_TABLE, "rank VARCHAR(50) NOT NULL PRIMARY KEY, _group VARCHAR(255), _prefix VARCHAR(255), _suffix VARCHAR(255), _message VARCHAR(50), _format VARCHAR(255)");
			getSQL().createTableAsync(Config.CENSOR_TABLE, "badword VARCHAR(255) NOT NULL PRIMARY KEY, _replace VARCHAR(255)");
			...
			getSQL().createTableAsync(Config.SERVER_FLAGS_TABLE, "server VARCHAR(100) NOT NULL, flag VARCHAR(50) NOT NULL, _value BIT(1) NOT NULL, PRIMARY KEY (server, flag)");
			...
			if (getSQL().checkColumnExists(Config.CENSOR_TABLE, "replace")) {
				getSQL().update(StringUtil.format("ALTER TABLE {0} CHANGE `replace` _replace VARCHAR(255);", Config.CENSOR_TABLE));
				getSQL().update(StringUtil.format("ALTER TABLE {0} CHANGE `value` _value BIT(1) NOT NULL;", Config.USER_FLAGS_TABLE));
				getSQL().update(StringUtil.format("ALTER TABLE {0} CHANGE `value` _value BIT(1) NOT NULL;", Config.SERVER_FLAGS_TABLE));
				getSQL().update(StringUtil.format("ALTER TABLE {0} CHANGE `group` _group VARCHAR(255);", Config.FORMAT_TABLE));
				getSQL().update(StringUtil.format("ALTER TABLE {0} CHANGE `prefix` _prefix VARCHAR(255);", Config.FORMAT_TABLE));
				getSQL().update(StringUtil.format("ALTER TABLE {0} CHANGE `suffix` _suffix VARCHAR(255);", Config.FORMAT_TABLE));
				getSQL().update(StringUtil.format("ALTER TABLE {0} CHANGE `message` _message VARCHAR(50);", Config.FORMAT_TABLE));
				getSQL().update(StringUtil.format("ALTER TABLE {0} CHANGE `format` _format VARCHAR(255);", Config.FORMAT_TABLE));
			}
